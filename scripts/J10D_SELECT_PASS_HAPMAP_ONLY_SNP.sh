...
CMD=$CMD' -o '$CORE_PATH'/'$IN_PROJECT'/MULTI_SAMPLE/VARIANT_SUMMARY_STAT_VCF/'$PREFIX'.BEDsuperset.VQSR.SNP.HAPMAP.SAMPLES.PASS.vcf'

END_HAPMAP_SNP_PASS=`date '+%s'`

HOSTNAME=`hostname`

echo $IN_PROJECT",J01,HAPMAP_SNP_PASS,"$HOSTNAME","$START_HAPMAP_SNP_PASS","$END_HAPMAP_SNP_PASS \
