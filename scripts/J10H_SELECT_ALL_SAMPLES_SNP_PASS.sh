...
CMD=$CMD' -o '$CORE_PATH'/'$IN_PROJECT'/MULTI_SAMPLE/VARIANT_SUMMARY_STAT_VCF/'$PREFIX'.BEDsuperset.VQSR.SNP.ALL.SAMPLES.PASS.vcf'

END_ALL_SNP_PASS=`date '+%s'`

HOSTNAME=`hostname`

echo $IN_PROJECT",J01,ALL_SNP_PASS,"$HOSTNAME","$START_ALL_SNP_PASS","$END_ALL_SNP_PASS \
